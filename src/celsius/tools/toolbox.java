...
public class toolbox {
    ...
    /**
     * Returns the number of pages of (protocol,MsgPrefix,originalfile,plaintxt)
     */
    public static int ReadNumberOfPagesOf(MsgLogger Msg1,String TI,String s,String t) throws IOException {
        Msg1.repS(TI+"Reading Number of Pages :: "+s);
        ...
        try {
            ...
            if (s.toLowerCase().endsWith(".ps.gz")) {
                ...
                while ((br.ready()) && (tmp.indexOf("%%Pages")<0)) tmp=br.readLine();
                if (tmp==null) {
                    Msg1.repS(TI+"No plain text file found.");
                    return(0);
                }
                if (tmp.indexOf("%%Pages")>-1) {
                    tmp=Parser.CutTill(Parser.CutFrom(tmp," ")," ");
                    try {
                        pages=(new Integer(tmp)).intValue();
                    } catch (NumberFormatException e) {
                        // %%Pages entry not working.
                        // Looking for another entry
                        Msg1.repS(TI+"First %%Pages entry does not work, looking for second one in "+s);
                        tmp=br.readLine();
                        while ((br.ready()) && (tmp.indexOf("%%Pages")<0)) tmp=br.readLine();
                        // second one found?
                        if (tmp.indexOf("%%Pages")>-1) {
        ...
        } catch (IOException e) { Msg1.repS(TI+"Error reading pages: "+e.toString()); }
        Msg1.repS(TI+"pages read:"+String.valueOf(pages));
        return(pages);
    }

    public static String getFirstPage(Item doc) {
        String fn=doc.get("plaintxt");
        if (fn==null) return("");
        return(getFirstPage(fn));
    }
    ...
    public static void centerFrame(ViewerText frame) {
