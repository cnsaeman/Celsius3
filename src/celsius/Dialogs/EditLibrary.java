...
import celsius.KeyValueTableModel;
...
public class EditLibrary extends javax.swing.JDialog {
    ...
    public EditLibrary(MainFrame mf, Library lib) {
        ...
        for (String t : RSC.LibraryFields) {
            KVTM.addRow(Parser.LowerEndOfWords(t), Lib.MainFile.get(t));
        }
    ...
    private void initComponents() {
        ...
        jTMain.setFont(new java.awt.Font("Arial", 0, RSC.guiScale(11)));
        jTMain.getTableHeader().setFont(new java.awt.Font("Arial", 0, RSC.guiScale(11)));
        jTMain.setRowHeight(RSC.guiScale(16));
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addComponent(jScrollPane1, javax.swing.GroupLayout.DEFAULT_SIZE, RSC.guiScale(596), Short.MAX_VALUE)
        );
        jPanel1Layout.setVerticalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel1Layout.createSequentialGroup()
                .addComponent(jScrollPane1, javax.swing.GroupLayout.DEFAULT_SIZE, RSC.guiScale(292), Short.MAX_VALUE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
    ...
    private void ApplyNameActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ApplyNameActionPerformed
        ...
        for (int i=0; i<RSC.LibraryFields.length;i++) {
            ...
            if ((value!=null) && !value.equals(Lib.MainFile.get(RSC.LibraryFields[i]))) {
                done=false;
                if (RSC.LibraryFields[i].equals("name")) {
                    Lib.name = value;
                    Lib.MainFile.put("name", Lib.name);
                    DefaultComboBoxModel DCBM = (DefaultComboBoxModel) MF.jCBLibraries.getModel();
                    int ind = MF.RSC.currentLib;
                    DCBM.removeElementAt(ind);
                    DCBM.insertElementAt(Lib.name, ind);
                    MF.switchToLibrary(Lib);
                    MF.jMCopyToDiff.getItem(i).setText(Lib.name);
                    MF.jMCopyToDiff1.getItem(i).setText(Lib.name);
                    MF.updateStatusBar(false);
                    done=true;
                    changed=true;
                }
                if (RSC.LibraryFields[i].equals("index")) {
                    toolbox.Warning(this,"Changing the fields in the library's main index requires a large amount of rewriting of the index files.\nThis operation is currently not possible", "Not possible");
                    KVTM.setValueAt(Lib.MainFile.get(RSC.LibraryFields[i]),i,1);
                    done=true;
                }
                if (!done) {
                    if (RSC.LibraryFields[i].startsWith("column")) {
                        if (Parser.HowOftenContains(value, "|")==Parser.HowOftenContains(Lib.MainFile.get("tablecolumns"), "|")) {
                            Lib.MainFile.put(RSC.LibraryFields[i], value);
                            changed=true;
                        } else {
                            toolbox.Warning(this,"The number of elements in the list \""+RSC.LibraryFields[i]+"\" does not match that of the table columns.", "Not possible:");
                            KVTM.setValueAt(Lib.MainFile.get(RSC.LibraryFields[i]),i,1);
                        }
                    } else {
                        Lib.MainFile.put(RSC.LibraryFields[i], value);
        ...
        if (changed) {
            Lib.Hide=new ArrayList<String>(Arrays.asList(Lib.listOf("hide")));
            try {
                Lib.MainFile.writeBack();
            } catch (IOException ex) {
                RSC.Msg1.printStackTrace(ex);
            }
            Lib.getFieldsFromMainFile();
        }
    ...
    private void jBtnEditCSSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBtnEditCSSActionPerformed
        String css="";
        for (String r : Lib.StyleSheetRules)
            css+="\n"+r;
        if (css.length()>0) css=css.substring(1);
        MultiLineEditor MLE=new MultiLineEditor(MF.RSC,"Edit HTML Stylesheet",css);
        MLE.setVisible(true);
        if (!MLE.cancel) {
            if (Lib.MainFile.get("style")==null) {
                Lib.MainFile.put("style","LD::style.css");
            } else {
                if (!(new File(Lib.completeDir(Lib.MainFile.get("style"),""))).exists())
                    Lib.MainFile.put("style","LD::style.css");
            }
            try {
                TextFile TF=new TextFile(Lib.completeDir(Lib.MainFile.get("style"),""),false);
                TF.putString(MLE.text);
                TF.close();
                Lib.loadStyleSheetRules();
                Lib.setChanged(true);
            } catch (IOException e) {
                MF.Msg1.printStackTrace(e);
                toolbox.Warning(this,"Error writing style file: "+e.toString(),"Warning:");
            }
        }
    ...
    private void jBtnManagePluginsActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBtnManagePluginsActionPerformed
        (new EditLibraryPlugins(MF,RSC.getCurrentSelectedLib())).setVisible(true);
        KVTM.clear();
        for (String t : RSC.LibraryFields) {
            KVTM.addRow(Parser.LowerEndOfWords(t), Lib.MainFile.get(t));
        }
    ...
    private void jBtnEditHTMLtemplateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBtnEditHTMLtemplateActionPerformed
        ...
        String template=Lib.getDisplayString(iM);
        MultiLineEditor MLE=new MultiLineEditor(RSC,"Edit HTML template for "+jCBHTMLtemplate.getSelectedItem(),template);
        MLE.setVisible(true);
        if ((!MLE.cancel) && (iM!=0)) {
            Lib.setDisplayString(iM, MLE.text);
        } else {
            if (!MLE.cancel) toolbox.Warning(this,"The default template cannot be modified.", "No changes applied");
        }
